...
async def kpi(days: int = 30, admin=Depends(require_role("admin"))):
    row = await pool.fetchrow("""
        SELECT
            COALESCE(SUM(CASE WHEN p.status = 'CONFIRMED' AND p.created_at >= CURRENT_DATE - $1 THEN p.amount END), 0) as revenue,
            (SELECT COUNT(*) FROM users WHERE created_at >= CURRENT_DATE - $1) as new_members,
            (SELECT COUNT(*) FROM subscriptions WHERE status = 'EXPIRED' AND updated_at >= CURRENT_DATE - $1) as churned,
...
async def funnel(days: int = 30, admin=Depends(require_role("admin"))):
    ...
    teaser_clicks = await pool.fetchval(
        "SELECT COUNT(*) FROM teaser_clicks WHERE created_at >= CURRENT_DATE - $1", days)
    trial_purchases = await pool.fetchval("""
        SELECT COUNT(*) FROM payments p JOIN packages pk ON p.package_id = pk.id
        WHERE p.status = 'CONFIRMED' AND pk.tier = 'TIER_99' AND p.created_at >= CURRENT_DATE - $1
    """, days)
