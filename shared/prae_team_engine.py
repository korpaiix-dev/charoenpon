...
# ===== Sprint 2.5: DB-backed prompt with async cache =====
import time as _t
import asyncio as _asyncio
_prompt_cache = {'val': None, 'expires': 0}

async def _async_load_active_prompt() -> str:
    """Async DB lookup for active Prae prompt.
    Returns SYSTEM_PROMPT constant if no active row or query fails.
    Cached for 60 seconds."""
    now = _t.time()
    if _prompt_cache['val'] and _prompt_cache['expires'] > now:
        return _prompt_cache['val']
    content = None
    try:
        from shared.database import get_session
        ...
        async with get_session() as s:
            r = (await s.execute(
                _t_sql("SELECT content FROM ai_prompts WHERE name='prae' AND is_active=TRUE ORDER BY version DESC LIMIT 1")
            )).first()
            if r:
                content = r.content
    except Exception:
    ...
    _prompt_cache['val'] = content
    _prompt_cache['expires'] = now + 60
    return content

def _load_active_prompt() -> str:
    """Sync wrapper: use cache if available, else return constant.
    Safe to call from sync code (won't block on DB)."""
    now = _t.time()
    if _prompt_cache['val'] and _prompt_cache['expires'] > now:
        return _prompt_cache['val']
...
"""
    ...
    messages = [
        {"role": "system", "content": (await _async_load_active_prompt()) + sys_extra},
