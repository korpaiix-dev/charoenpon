"""slip_handler.py — extracted from bots/sales_bot/handlers/payment.py.

Holds the photo-slip processing pipeline:
- handle_photo_slip — main slip image handler (PHOTO message → OCR/AI → Slip2Go → approve/reject)
- _get_effective_price — per-tier pricing with promo/comeback overrides (delegates to shared.pricing)
- _send_welcome_referral_dm — DM new VIPs about referral
- _build_admin_approve_kb — admin approve inline keyboard from shared.pricing.approve_buttons

Round E final extraction (Strangler Fig). Logic UNCHANGED — only moved.
"""
...
from bots.sales_bot.payment_util.approve import _approve_payment, WELCOME_REFERRAL_DM
