...
async def check_and_grant_referral_reward(referrer_user_id: int) -> int | None:
    """Check referral count and grant reward by extending subscription end_date.

    Returns reward_days granted, or None if no new reward.
    """
    async with get_session() as session:
        ...
        # Already granted rewards
        granted_result = await session.execute(
            text("""
                SELECT COALESCE(SUM(reward_days), 0) FROM referrals
                WHERE referrer_user_id = :uid AND status = 'REWARDED'
            """),
            {"uid": referrer_user_id},
        )
        already_granted = granted_result.scalar() or 0

        # Calculate expected total reward for current count
        expected_total = 0
        for req_count, reward_days in REWARD_TIERS:
            if total_count >= req_count:
                expected_total = reward_days
                break

        # Calculate what's new to grant
        # Per-referral base: 7 days each
        base_reward = total_count * 7
        # Milestone bonuses
        milestone_bonus = 0
        if total_count >= 5:
            milestone_bonus = 90 - (5 * 7)  # 90 - 35 = 55 extra
        elif total_count >= 3:
            milestone_bonus = 30 - (3 * 7)  # 30 - 21 = 9 extra

        total_expected_days = base_reward + milestone_bonus
        new_days = total_expected_days - already_granted

        if new_days <= 0:
            return None

        # Extend active subscription
        extend_result = await session.execute(
            text("""
                ...
                RETURNING id
            """),
            {"uid": referrer_user_id, "days": new_days},
        )
        updated_sub = extend_result.fetchone()

        if not updated_sub:
            logger.warning("No active subscription to extend for user %d", referrer_user_id)
            return None

        # Mark referrals as REWARDED
        await session.execute(
            text("""
                UPDATE referrals SET status = 'REWARDED', reward_days = 7
                WHERE referrer_user_id = :uid AND status = 'COMPLETED'
            """),
            {"uid": referrer_user_id},
        )
        await session.commit()

    logger.info("Granted %d referral reward days to user %d (total refs: %d)", new_days, referrer_user_id, total_count)
    return new_days
