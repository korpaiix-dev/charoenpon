...
# ── แพร v2 STAGING — only these tg_ids see new AI ──
PRAE_V2_WHITELIST = {
    8502597269,   # บอส (เจริญพร)
    7387557933,   # admin #2
}
...
async def handle_support_text(
    ...
    # ── HYBRID SOS: notify admin in background if SOS keyword detected ──
    try:
        from shared.prae_sos_hybrid import is_sos, trigger_sos_workflow
        if is_sos(user_text):
            await trigger_sos_workflow(
                telegram_id=user.id,
                user_text=user_text,
            ...
            # Continue to AI — AI will guide user (e.g. suggest /getlink)
    except Exception as _e_sos:
        logger.warning("SOS hybrid failed (non-fatal): %s", _e_sos)

    # ── PUBLIC: route ALL text messages to prae v2 ──
    try:
        from shared.prae_engine import reply_to_user, ensure_memory_table
        await ensure_memory_table()
        result = await reply_to_user(user.id, user_text)
        ...
        if reply:
            ...
            logger.info(
                "prae_v2 reply: tg=%s intent=%s confidence=%s handoff=%s tools=%s cost=$%.4f",
                user.id, result.get("intent"), result.get("confidence"),
                result.get("should_handoff"), result.get("tools_used"),
                result.get("cost_usd", 0),
            )
            return
    except Exception as _e_v2:
        logger.exception("prae v2 failed for tg=%s: %s", user.id, _e_v2)
